...
export async function createAuditLog(data) {
  ...
  const sql =
    "INSERT INTO audit_logs (actor, role, action, details, severity, user_agent, entity_type, entity_id, ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
  await dbRun(sql, [
    actor,
    role,
    action,
...
export async function countAuditLogs(options) {
  ...
  if (search) {
    whereClauses.push(
      "(actor LIKE ? OR action LIKE ? OR details LIKE ? OR ip LIKE ? OR entity_type LIKE ? OR entity_id LIKE ?)"
    )
    const term = "%" + search + "%"
...
export async function listAuditLogs(options) {
  ...
  const sortOrder =
    String(opt.sortOrder || "DESC").toUpperCase() === "ASC" ? "ASC" : "DESC"
...
export async function createGlobalAuditLog(data) {
  ...
  const sql =
    "INSERT INTO global_audit_logs (actor, role, office_id, action, details, severity, ip) VALUES (?, ?, ?, ?, ?, ?, ?)"
...
export async function listGlobalAuditLogs(options = {}) {
  ...
  if (search) {
    whereClauses.push(
      "(actor LIKE ? OR action LIKE ? OR details LIKE ? OR ip LIKE ?)"
    )
    const term = "%" + search + "%"
...
/**
 * Calculates stats for global audit logs (including 7-day trend data)
 */
export async function getGlobalAuditLogStats(options = {}) {
  ...
  const mainRows = await sysDbAll(mainQuery, params)
  const stats =
    mainRows && mainRows.length > 0
      ? mainRows[0]
      : {
  ...
  if (whereClauses.length > 0) {
    trendQuery += " AND " + whereClauses.join(" AND ")
  }

  trendQuery += " GROUP BY day ORDER BY day ASC"

  const trendRows = await sysDbAll(trendQuery, params)
