          ...
          <CardContent className="grid min-h-0 gap-4 overflow-auto p-4 xl:grid-cols-2">
            {selected ? <>
              ...
              <div className="space-y-3">
                <label className="block text-xs font-semibold text-gray-600 dark:text-zinc-300">Student assignment<select className="mt-1 h-9 w-full rounded-md border border-gray-200 bg-white px-2 text-sm dark:border-white/10 dark:bg-zinc-900" value={selected.proposed_student_no || ""} onChange={(event) => update({ studentNo: event.target.value }).catch((error) => showToast({ title: "Save failed", description: error.message }, true))}><option value="">Unmatched</option>{students.map((student) => <option key={student.studentNo || student.student_no} value={student.studentNo || student.student_no}>{student.name} ({student.studentNo || student.student_no})</option>)}</select></label>
