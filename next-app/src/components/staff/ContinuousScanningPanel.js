    ...
    <div className="mx-6 my-2.5 rounded-2xl border border-blue-200/70 bg-blue-50/50 p-3.5 dark:border-blue-500/20 dark:bg-blue-950/20">
      ...
      {showCompleted && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label="Completed scans">
        <div className="flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-white/10 dark:bg-zinc-900">
          ...
          <div className="flex flex-wrap gap-2 border-b border-gray-100 p-4 dark:border-white/5"><Input value={completedQuery} onChange={(event) => { setCompletedQuery(event.target.value); setCompletedPage(0); }} placeholder="Search filename, student, or type" className="h-9 min-w-[220px] flex-1 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-zinc-800 px-3 text-xs font-normal text-gray-900 dark:text-zinc-100 placeholder:text-gray-400 dark:placeholder:text-zinc-500 shadow-none focus-visible:outline-none focus-visible:border-pup-maroon focus-visible:ring-1 focus-visible:ring-pup-maroon dark:focus-visible:border-red-500/80 dark:focus-visible:ring-red-500/80" /><div className="w-[180px] shrink-0"><Select value={completedStatus} onChange={(event) => { setCompletedStatus(event.target.value); setCompletedPage(0); }} className="h-9 rounded-xl border border-gray-200 bg-white px-3 text-xs font-normal text-gray-700 shadow-none outline-none focus-visible:outline-none focus-visible:border-pup-maroon focus-visible:ring-1 focus-visible:ring-pup-maroon dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-200"><option value="">All review statuses</option><option value="Needs Review">Needs Review</option><option value="Conflict">Conflict</option><option value="Duplicate">Duplicate</option><option value="Rejected">Rejected</option><option value="Confirmed">Confirmed</option></Select></div></div>
