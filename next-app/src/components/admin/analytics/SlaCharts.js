        ...
        <div className="space-y-1.5">
          {payload.map((entry, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-gray-700 dark:text-zinc-200">{entry.name}:</span>
              <span className="text-xs font-black text-gray-900 ml-auto dark:text-zinc-50">{entry.value}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="rounded-lg border border-gray-100 bg-white p-3 shadow-xl dark:border-white/10 dark:bg-card dark:shadow-none">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-gray-700 dark:text-zinc-200">{entry.name}:</span>
          <span className="text-xs font-black text-gray-900 dark:text-zinc-50">{entry.value} requests</span>
        </div>
      </div>
    )
  }
            ...
            {pieData.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-inter font-black text-gray-900 dark:text-zinc-50">{d.value}</span>
                    <span className="text-[10px] font-bold text-gray-400 dark:text-zinc-500">{percent}%</span>
                  </div>
                </div>
              )
            })}
              ...
              data.topDocTypes.map((dt, i) => (
                ...
                >
                  <div className="flex items-center gap-2 overflow-hidden">
                    <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-white text-[10px] font-bold text-gray-500 shadow-sm dark:border-white/10 dark:bg-card dark:text-zinc-400 dark:shadow-none">
                      {i + 1}
                    </div>
                    <span className="truncate text-sm font-bold text-gray-800 dark:text-zinc-100">
                      {dt.name}
                    </span>
                  </div>
