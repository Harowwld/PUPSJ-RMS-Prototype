...
export default function RecognitionTemplatesTab({ showToast }) {
  ...
  async function saveTemplate() {
    ...
    setSaving(true)
    try {
      const payload = { documentTypeId, name: templateName, version: selectedTemplateId ? Number(version) + 1 : Number(version), pageIndex, rotation, regions }
      ...
      <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)_280px]">
        <div className="space-y-4 rounded-brand border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-white/5">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">Click a field, then draw its highlight box. Existing boxes stay visible.</p>
          </div>
          <div className="grid grid-cols-1 gap-2">
            {FIELDS.map(([key, label]) => {
              const plotted = regions[key].width > 0 && regions[key].height > 0
              return <Button key={key} type="button" variant={activeField === key ? "default" : "outline"} onClick={() => { setActiveField(key); setDraftRegion(null) }} className="justify-between" style={activeField === key ? { backgroundColor: COLORS[key] } : undefined}>
                <span>{label}</span>
        ...
        <div className="min-w-0 rounded-brand border border-gray-200 bg-gray-100 p-4 dark:border-white/10 dark:bg-zinc-900">
          ...
          {pageImage && <div className="mt-4 grid gap-3 rounded-brand border border-gray-200 bg-white p-3 dark:border-white/10 dark:bg-card sm:grid-cols-[minmax(0,1fr)_180px]">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">{FIELDS.find(([key]) => key === activeField)?.[1]}: {previewText || "No OCR text detected inside this box yet."}</p>
            </div>
