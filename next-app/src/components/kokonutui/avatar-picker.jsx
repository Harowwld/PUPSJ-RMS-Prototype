...
export default function ProfileSetup({
  ...
  const handleSubmit = () => {
    if (mode === "settings") {
      if (onComplete) onComplete({ avatarId: selectedAvatar.id });
    } else {
  ...
  const isValid = mode === "settings" ? true : username.trim().length >= 3;
  const showError = mode === "settings" ? false : username.trim().length > 0 && username.trim().length < 3;
