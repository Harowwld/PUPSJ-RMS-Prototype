...
export async function PATCH(req, ctx) {
  ...
  const existing = await queryOne("SELECT id, status FROM document_requests WHERE id = $1 AND office_id = 'registrar'", [id]);
  ...
  const updated = await queryOne("UPDATE document_requests SET status = $1, updated_at = NOW(), updated_by = $2 WHERE id = $3 AND office_id = 'registrar' RETURNING *", [status, access.userId || null, id]);
