...
async function checkAndRunScheduledBackups() {
  const { dbAll, dbRun } = await import("./src/lib/postgresCompat.js");
  ...
  for (const row of rows) {
    try {
      ...
      let record;
      if (isSystem) {
        const { executeSystemBackup } = await import("./src/lib/backupsRepo.js");
        record = await executeSystemBackup({
          actorId: schedule.createdBy || null,
        });
      } else {
        const { executeOfficeBackup } = await import("./src/lib/backupsRepo.js");
      ...
      // Write audit log
      try {
        const { createAuditLog } = await import("./src/lib/auditLogsRepo.js");
